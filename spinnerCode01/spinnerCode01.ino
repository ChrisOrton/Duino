double delayTime = 1.1; //these value may change acc. to the length of the word in the POV display
double charBreak = 2.7; //these value may change acc. to the length of the word in the POV display
...
void displayChar(char ch){
  ...
  if (ch == ' '){for (int i = 0; i <5; i++){displayLine(spc[i]);delay(delayTime);}displayLine(0);}
  if (ch == 'X'){for (int i = 0; i <5; i++){displayLine(X[i]);delay(delayTime);}displayLine(0);}
  delay(charBreak);
}
...
void loop()
{
  displayString("hello world  XoXo  ");
}
